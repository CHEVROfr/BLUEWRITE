...
Note = function(noteObj, sess) {
    ...
    this.getHtmlListDiv = function(withBook) {
        ...
        if(withBook) {
            ...
            html += '<a href="/books?bid=' + this.book.bid + '#b_' + this.book.bid + '" class="noteBook"><svg><use xlink:href="/files/cicons/cicons.svg?v=9#book"></use></svg>' + htmlspecialchars(bookName) + '</a>'
        }        
...
search = (query) => {
    if(isSearching) {
        ...
        isSearching = true
        content.innerHTML = '<div id="loading"><svg><use xlink:href="/files/cicons/cicons.svg?v=9#loading"></use></svg></div>'
